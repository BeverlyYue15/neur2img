...
class FluxKontextPipeline(
    ...
    def prepare_latents(
        ...
        height_cond = 2 * (int(self.cond_size) // (self.vae_scale_factor * 2))
        width_cond = 2 * (int(self.cond_size) // (self.vae_scale_factor * 2))
        ...
        latents_to_concat = []
        latents_ids_to_concat = [latent_ids]
        ...
        if subject_image is not None:
            ...
            subject_image_latents=subject_image
            ...
            latent_subject_ids = self._prepare_latent_image_ids(batch_size, height_cond // 2, width_cond // 2, device, dtype)
        ...
        subject_cond_latents = torch.concat(latents_to_concat, dim=-2)
        latent_image_ids = torch.concat(latents_ids_to_concat, dim=-2)
        
        return subject_cond_latents, latents, image_latents, latent_image_ids
    ...
    def __call__(
        ...
        if sub_number > 0:
            ...
            for subject_image in subject_images:
                ...
                subject_image = subject_image.to(dtype=torch.bfloat16)
                subject_image = subject_image.repeat_interleave(num_images_per_prompt, dim=0)
        ...
        if image is not None and not (isinstance(image, torch.Tensor) and image.size(1) == self.latent_channels):
            ...
            image = self.image_processor.resize(image, 512, 512)
            image = self.image_processor.preprocess(image, 512, 512)
        # 4. Prepare latent variables
        num_channels_latents = self.transformer.config.in_channels // 4
        cond_latents, latents, image_latents, latent_ids = self.prepare_latents(
            image,
